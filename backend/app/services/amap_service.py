...
class AmapService:
    ...
    def search_poi(self, keywords: str, city: str, citylimit: bool = True) -> List[POIInfo]:
        ...
        try:
            ...
            # 解析MCP工具返回的JSON结果
            try:
                # 提取JSON部分（MCP工具返回格式可能包含其他信息）
                import re
                import json
                json_match = re.search(r'\{.*\}', result, re.DOTALL)
                if json_match:
                    poi_data = json.loads(json_match.group())
                    # 将JSON数据转换为POIInfo对象列表
                    return [
                        POIInfo(
                            id=item.get('id', ''),
                            name=item.get('name', ''),
    ...
    def plan_route(
        ...
        try:
            ...
            try:
                ...
                import re
                import json
                # 改进正则表达式，确保匹配完整的JSON对象
                json_pattern = r'\{[\s\S]*\}'
                json_match = re.search(json_pattern, result)
                if json_match:
                    route_data = json.loads(json_match.group())
                    print(f"提取到的JSON数据: {route_data}")
                    
                    # 检查是否有错误
                    if route_data.get("error"):
                        print(f"MCP工具返回错误: {route_data['error']}")
                        return {
                            "distance": 0.0,
                            "duration": 0,
                            "route_type": route_type,
                            "description": f"错误: {route_data['error']}"
                        }
                    ...
                    if route_data.get("routes"):
                        ...
                        if routes:
                            # 转换为数值类型
                            route_result["distance"] = float(routes[0].get("distance", 0))
                            route_result["duration"] = int(routes[0].get("duration", 0))
                            route_result["description"] = f"距离: {route_result['distance']}米, 预计时间: {route_result['duration']}秒"
                    elif route_data.get("route"):
                        route = route_data.get("route", {})
                        # 先尝试从route.paths获取（根据实际返回数据结构）
                        if route.get("paths"):
                            paths = route.get("paths", [])
                            if paths:
                                route_result["distance"] = float(paths[0].get("distance", 0))
                                route_result["duration"] = int(paths[0].get("duration", 0))
                        # 再尝试直接从route获取
                        elif route.get("distance"):
                            route_result["distance"] = float(route.get("distance", 0))
                            route_result["duration"] = int(route.get("duration", 0))
                        route_result["description"] = f"距离: {route_result['distance']}米, 预计时间: {route_result['duration']}秒"
                    # 直接从顶层获取
                    elif route_data.get("distance"):
                        route_result["distance"] = float(route_data.get("distance", 0))
                        route_result["duration"] = int(route_data.get("duration", 0))
                        route_result["description"] = f"距离: {route_result['distance']}米, 预计时间: {route_result['duration']}秒"
