...
7. **必须包含预算信息**:
   ...
   - 预算汇总(budget)包含各项总费用
8. 数组的每个元素之间必须有逗号分隔
9. 数字值只能是数字，不能包含表达式或计算，不能中间含有空格，对异常值要修正
10. 必须严格按照模板生成，所有的字段多不能缺少
11. budget字段不能包含表达式，示例："budget": {
    "total_attractions": 180,
    "total_hotels": 1200,
    "total_meals": 480,
    "total_transportation": 200,
    ...
    def plan_trip(self, request: TripRequest) -> TripPlan:
        ...
        try:
            ...
            for attempt in range(max_retries + 1):
                try:
                    print(f"尝试生成计划 (第{attempt + 1}/{max_retries + 1})...")
                    ...
                    print(f"⚠️  LLM调用失败: {str(llm_error)}")
                    if attempt < max_retries:
                        print(f"正在重试... ({attempt + 1}/{max_retries})")
                    else:
                        print(f"已达到最大重试次数，使用备用方案")
                        break

            # 所有重试都失败，使用备用计划
            print(f"使用备用方案生成旅行计划...")
...
"""
    ...
    def _parse_response(self, response: str, request: TripRequest) -> TripPlan:
        ...
        try:
            ...
            # 情况2: 包含 ``` 代码块
            if json_str is None and "```" in clean_response:
            ...
            # 情况3: 直接包含JSON对象
            if json_str is None and "{" in clean_response and "}" in clean_response:
                # 找到第一个{和最后一个}
            ...
            try:
                ...
                print(f"✅ TripPlan对象创建成功")
                
                return trip_plan
                
            except json.JSONDecodeError as json_error:
                print(f"⚠️  JSON格式错误: {str(json_error)}")
                print(f"响应片段: {json_str[:200]}...")
                return self._create_fallback_plan(request)
            except Exception as model_error:
                print(f"⚠️  TripPlan模型错误: {str(model_error)}")
                return self._create_fallback_plan(request)
    ...
    def _create_fallback_plan(self, request: TripRequest) -> TripPlan:
        ...
        # 计算预算
        total_attractions = 60 * 2 * request.travel_days  # 2个景点/天，60元/景点
        total_hotels = 400 * request.travel_days  # 400元/天
        total_meals = (30 + 50 + 80) * request.travel_days  # 三餐/天
        total_transportation = 200  # 总交通费用
